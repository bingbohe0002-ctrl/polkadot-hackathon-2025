...
const adminWallet = new ethers.Wallet(ADMIN_PK);
...
async function makeAdminClientIfNeeded() {
  ...
  const signer: Signer = {
    type: "EOA",
    getIdentifier: async () => ({
      identifier: adminWallet.address.toLowerCase(),
    ...
    signMessage: async (message: string | Uint8Array | unknown): Promise<Uint8Array> => {
      ...
      // 使用 ethers 的 signMessage，返回 hex 签名，然后转换为 Uint8Array
      const sigHex = await adminWallet.signMessage(msgBytes);
      return ethers.utils.arrayify(sigHex);
    },
  };

  adminClient = await Client.create(signer, { env: XMTP_ENV });
  console.log("admin XMTP client 初始化完成，address:", adminWallet.address);
  return adminClient;
}
...
app.post("/api/invite/redeem", async (req, res) => {
  try {
    ...
    // 1) 验签：使用 ethers 的 verifyMessage 恢复签名地址
    let recovered: string;
    try {
      recovered = ethers.utils.verifyMessage(info.nonce, signature);
    } catch (e) {
